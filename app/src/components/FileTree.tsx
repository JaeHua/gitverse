...
export default function FileTree({
  ...
  useEffect(() => {
    ...
    const sh = defs.append('filter').attr('id', 'sh2').attr('x', '-30%').attr('y', '-30%').attr('width', '160%').attr('height', '160%')
    sh.append('feDropShadow').attr('dx', 0).attr('dy', 1).attr('stdDeviation', 1.5).attr('flood-color', '#000').attr('flood-opacity', 0.04)
    defs.append('marker').attr('id', 'arr2').attr('viewBox', '0 -4 8 8').attr('refX', 7).attr('refY', 0)
    ...
    const branches = branchG.selectAll('path').data(root.links() as d3.HierarchyPointLink<TreeNode>[]).join('path')
      .attr('fill', 'none').attr('stroke', C.branch).attr('stroke-width', 1).attr('stroke-linecap', 'round')

    if (playing && fwd) {
      branches.each(function (d) {
    ...
    g.append('g').selectAll('path')
      ...
      .attr('stroke-opacity', 0.18).attr('marker-end', 'url(#arr2)').style('pointer-events', 'none')

    // Nodes
    const nodeG = g.append('g').selectAll('g').data(desc).join('g')
    ...
    nodeG.filter(d => !d.data.fileNode && d.depth >= 1)
      ...
      .attr('fill', C.dirFill).attr('stroke', C.branch).attr('stroke-width', 0.5).attr('filter', 'url(#sh2)')
    nodeG.filter(d => !d.data.fileNode && d.depth >= 1)
    ...
    fileNodes.append('circle')
      ...
      .attr('fill', d => heatFill(d.data.heat)).attr('filter', 'url(#sh2)')
    ...
    fileNodes.append('text')
      ...
      .attr('opacity', playing && fwd ? 0 : 0.5)
    if (playing && fwd) {
      fileNodes.select('text')
        .transition().delay(ANIM * 0.6).duration(800).attr('opacity', 0.5)
    }
    ...
    fileNodes.on('click', (_e, d) => {
      if (d.data.fileNode) onNodeSelect(d.data.path === selectedNodeId ? null : d.data.path)
    })

    // Selection
    fileNodes.filter(d => d.data.path === selectedNodeId).select('circle')
      .attr('stroke', C.select).attr('stroke-width', 2.5)
    fileNodes.filter(d => d.data.path === selectedNodeId).select('text')
